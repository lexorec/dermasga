---
title: "Caída del cabello y tricología"
seo_title: "Tratamiento de la caída del cabello en Machala | DERMASGA"
description: "Diagnóstico y tratamiento de la caída del cabello en hombres, mujeres y niños en Machala, con tricoscopía y protocolos personalizados. Dra. Marcela Astudillo G."
order: 2
whatsapp_message: "Hola Dra. Astudillo, quisiera agendar una cita por caída del cabello."
eyebrow: "Tricología · DERMASGA"
heading: "Caída del cabello y tricología en Machala"
lead: "Atención especializada para hombres y mujeres adultos, y para niños que presenten problemas de pérdida de cabello, con un diagnóstico completo y protocolos combinados y personalizados."
facts:
  - label: "Pacientes"
    value: "Hombres, mujeres y niños"
  - label: "Diagnóstico"
    value: "Tricoscopía, luz de Wood y prueba de tracción"
  - label: "Primeros cambios"
    value: "Por lo general, desde el tercer mes de tratamiento"
sections:
  - question: "¿Qué tipos de pérdida de cabello se tratan en consulta?"
    answer: |-
      Manejamos diversas condiciones capilares, incluyendo:
    items:
      - "Alopecia androgenética"
      - "Efluvio telógeno"
      - "Alopecia areata"
      - "Alopecia frontal fibrosante"
      - "Liquen plano pilar"
      - "Foliculitis decalvante"
      - "Alopecia por tiñas"
  - question: "¿A quiénes están dirigidos los tratamientos capilares?"
    answer: |-
      Brindamos atención especializada tanto a hombres y mujeres adultos como a niños que presenten problemas de pérdida de cabello.
  - question: "¿Cómo se realiza el diagnóstico de las condiciones capilares?"
    answer: |-
      Llevamos a cabo una evaluación clínica exhaustiva que incluye:

      - **Tricoscopía:** examen detallado del cuero cabelludo con aumento.
      - **Análisis con luz de Wood.**
      - **Medición del volumen capilar** con cinta métrica.
      - **Prueba de tracción** (pull test).

      Dependiendo de los hallazgos de esta evaluación, solicitamos exámenes de sangre específicos para confirmar el diagnóstico.
  - question: "¿Qué tratamientos capilares se ofrecen en el consultorio?"
    answer: |-
      Diseñamos protocolos combinados y personalizados según las necesidades de cada paciente. Estos pueden incluir:

      - Tratamientos tópicos.
      - Medicación oral.
      - Terapia con láser capilar.
      - Mesoterapia capilar.
  - question: "¿En cuánto tiempo se empiezan a notar los resultados?"
    answer: |-
      El tiempo de evolución depende de la severidad y el tipo de condición de cada paciente. Sin embargo, por lo general, los primeros cambios positivos son visibles a partir del tercer mes de tratamiento continuo.
testimonials:
  - quote: "Tuve dermatitis seborreica, el cual generaba que mi cabello caiga y no volvía a salir nuevos cabellos, tras un mes y medio de tratamiento recibí el alta médica… Yo salí más que satisfecho."
    name: "J. C."
  - quote: "Sus consejos y tratamiento me ha ayudado a reparar y mantener mi piel y cabello muy saludables. Sin lugar a duda estoy feliz con los resultados."
    name: "S. M."
---
